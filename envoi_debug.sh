...
else echo -e "MUTE CALL HAVE BEEN DETECTED: \n" `grep "MUTE CALL" ~axadmin/mutecall/mutecall-\`echo ${dat}\` |tr \-s \'.\' \\n `
...
#mailx -v  -S smtp="10.162.168.195:25" -s "PRECHECK MATIN : `date +\"%d/%m/%Y\"` OMCR SE1 LYON" -a ~axadmin/mutecall/mutecall-`echo ${dat}`.csv lromeiro.ext@orange.com \
#<~axadmin/mutecall/mutecall-`echo ${dat}`.csv
