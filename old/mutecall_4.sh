...
if [ -z $dl ] || [ -z $ul ]
then echo "cell " $CI " not found in the OBSYNT GPRS, so we will use GPMRES raw file"
...
then echo -e "${RED}RESULT: that is mute calls. Alert ! Perform a reset of the module. CI=" $1${NC}
